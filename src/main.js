...
function renderWeather(weather) {
  try {
    $(".temperature").html(`${weather.main.temp} °C en ${weather.name}`);
    $(".feels_like").html(`Sensacion Termica: ${weather.main.feels_like} °C`);
    $(".min_temperature").html(`Minima: ${weather.main.temp_min} °C`);
    $(".max_temperature").html(`Maxima : ${weather.main.temp_max}°C`);
    $(".humidity").html(`Humedad ${weather.main.humidity}%`);
    $(".wind_speed").html(`Viento : ${weather.wind.speed} km/h `);
    $(".visibility").html(`Visibilidad ${weather.visibility}`);
    $(".pressure").html(`Presion : ${weather.main.pressure} hPa`);
    $(".weather_img").attr(
      "src",
  ...
  try {
    ...
    forecast[index].forecast.forEach((element) => {
      ...
      clone.querySelector(".hourly__extraInfo").innerHTML = `
        <p class="hourly__max">Maxima : ${element.main.temp_max} °C</p>
        <p class="hourly__min">Minima :${element.main.temp_min} °C</p>
        <p class="hourly__humidity"> Humedad :${element.main.humidity}% </p>
      `;
